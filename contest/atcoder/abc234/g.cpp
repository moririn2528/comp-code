...
namespace sol{
    ...
    void solve(){
        ...
        vector<LL> v1(n);
        cin>>v1;
        vector<mint> v2={1},v3={0,1};
        vector<P> va={{INF,-1}},vb={{-1,-1}};
        vector<mint> vc={0},vd={0};
        mint ma,mb;
        for(i=0;i<n;i++){
            while(va.back().first<=v1[i])va.pop_back(),vc.pop_back();
            while(vb.back().first>=v1[i])vb.pop_back(),vd.pop_back();
            ma=vc.back()+(v3[i+1]-v3[va.back().second+1])*v1[i];
            mb=vd.back()+(v3[i+1]-v3[vb.back().second+1])*v1[i];
            va.push_back({v1[i],i});
            vb.push_back({v1[i],i});
            vc.push_back(ma);
            vd.push_back(mb);
