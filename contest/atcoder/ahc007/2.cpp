...
namespace sol{
    ...
    void solve(){
        ...
        input();
        vector<int> used(M);
        const double max_coeff=3;
        const double start_coeff=1.6,end_coeff=2.0;
        for(i=0;i<M;cout<<used[i]<<endl,i++){
            ...
            if(ua.same(x,y))continue;
            for(j=i+1;j<M;j++){
                a=path[j].first,b=path[j].second;
                ua.merge(a,b);
                v1.push_back({dis[a][b],j});
            }
            if(!ua.same(x,y)){
                used[i]=1;
                continue;
            ...
            double coeff=start_coeff+(end_coeff-start_coeff)*i/M;
            if(cost*max_coeff<=l)continue;
            if(l<=cost*coeff){
                used[i]=1;
                continue;
            }
