...
namespace sol{
    ...
    void solve(){
        ...
        vector<vector<double>> real_cost_vec(M,vector<double>(N));

        for(int day=0;;day++){
            vector<int> output;
            vector<int> task_assign_pre(N,-1),v1;
            unordered_set<int> member_able;
            ...
            for(i=0,j=0;i<M;i++){
                ...
                if(j<max_rnd_task && get_double_rnd()<rnd_per && !v1.empty()){
                    ...
                    //optimize assign
                    member_able.insert(i);
                    for(int id:task_able){
                        real_cost_vec[i][id]=real_cost(i,id);
                    }
                }
            }
            
            for(int loop_count=0;!member_able.empty() && !task_able.empty();loop_count++){
                assert(loop_count<M);
                vector<double> opt_score(M,-1.0);//want to high
                vector<int> opt_id(M,-1);
                for(int task_id:task_able){
                    int id=-1;
                    double ct=INF;//cost
                    for(int mem_id:member_able){
                        da=real_cost_vec[mem_id][task_id];
                        if(da<ct){
                            ct=da;
                            id=mem_id;
                        }
                ...
                for(i=0;i<M;i++){
                    if(member_able.find(i)==member_able.end())continue;
                    int task_id=opt_id[i];
