...
namespace sol{
    ...
    void solve(){
        ...
        while(cin>>n){
            ...
            vector<vector<C>> vp;
            vector<vector<double>> score(n+2,vector<double>(2,1e9));
            for(i=0;i<n;i++){
                cin>>a>>b>>c;
                vc.push_back(Circle<double>(a,b,c));
            }
			vp.push_back(vector<C>{vc[0].p,vc[0].p});
            for(i=0;i+1<n;i++){
                vp.push_back(vc[i].cross_point(vc[i+1]));
                assert(vp.back().size()==2);
            }
			vp.push_back(vector<C>{vc.back().p,vc.back().p});
            score[0][0]=score[0][1]=0;
            for(i=1;i<=n;i++){
                for(j=0;j<2;j++){
                    for(k=0;k<i;k++){
                        for(p=0;p<2;p++){
                            SegLine<double> l1(vp[i][j],vp[k][p]);
                            for(q=k+1;q<i;q++){
                                SegLine<double> l2(vp[q][0],vp[q][1]);
                                if(!l1.is_cross(l2))break;
                            }
                            if(i<=q){
                                score[i][j]=min(score[i][j],score[k][p]+abs(vp[i][j]-vp[k][p]));
                            }
                        }
                    }
                }
            }
            cout<<min(score[n][0],score[n][1])<<endl;
        }
    }
}
