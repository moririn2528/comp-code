...
namespace sol{
    ...
    void solve(){
        ...
        string sa;
        cin>>sa;
        seg_tree<int> seg([](int a,int b){return a+b;},0,n);
        vector<int> v1(n);
        auto f=[&](int pos,int dif){
            if(pos+1<n && sa[pos]=='(' && sa[pos+1]==')'){
                seg.set(pos,seg.get(pos)+dif);
            }
            if(0<pos && sa[pos-1]=='(' && sa[pos]==')'){
                seg.set(pos-1,seg.get(pos-1)+dif);
            }
        };
        for(i=0;i+1<n;i++){
            if(sa[i]=='(' && sa[i+1]==')')seg.set(i,1);
        }
        for(i=0;i<m;i++){
            cin>>a;
            if(a==1){
